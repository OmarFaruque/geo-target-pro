  ...
  try {
    ...
    if (hiddenProductIds.length > 0) {
      ...
      hiddenProductHandles = (productJson.data?.nodes ?? [])
        .map((node: { handle?: string | null } | null) => node?.handle)
    ...
    console.error("[GeoTarget Pro] Failed to load public geotarget payload", error);

    return Response.json(
      {
        error: "Failed to load storefront config.",
      },
